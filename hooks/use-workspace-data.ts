...
function reorderTaskForMove(
  ...
  for (const task of withoutMovingTask) {
    if (!inserted && getTaskColumn(task) === newColumnId && targetColumnCount === destinationIndex) {
...
function normalizeTask(task: Task): Task {
  return {
    ...
    assignmentRequestedAt: task.assignmentRequestedAt ? new Date(task.assignmentRequestedAt as any) : null,
    assignmentRespondedAt: task.assignmentRespondedAt ? new Date(task.assignmentRespondedAt as any) : null,
    comments: (task.comments || []).map((comment) => ({
      ...comment,
      createdAt: comment.createdAt ? new Date(comment.createdAt as any) : new Date(),
      updatedAt: comment.updatedAt ? new Date(comment.updatedAt as any) : undefined,
    })),
...
export function useWorkspaceData() {
  ...
  const createTask = async (task: CreateTaskInput) => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Errore durante la creazione della task")
      throw err
    }
  }
  ...
  const deleteTaskAttachment = async (taskId: string, attachmentId: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Errore durante la rimozione dell'allegato")
      throw err
    }
  }
  ...
  const moveTask = async (taskId: string, newColumnId: string, options: MoveTaskOptions = {}) => {
    ...
    setTasks((current) => {
      ...
      return reorderTaskForMove(current, taskId, newColumnId, options.destinationIndex)
    })
    ...
    try {
      ...
      setTasks(previousTasks)
      setError(err instanceof Error ? err.message : "Errore durante lo spostamento della task")
      throw err
    }
  }
