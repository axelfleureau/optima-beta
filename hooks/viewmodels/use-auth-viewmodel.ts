  ...
  const permissionHelpers = useMemo(
    () => ({
      ...
      canViewResource: (resourceOwnerId: string) => {
        return user
          ? canViewResource(user.role, resourceOwnerId, user.id)
          : false;
      },
    }),
    [permissions, user],
...
export function useAuthStatus() {
  ...
  const loading = useSelector(
    (state: RootState) => state.auth.loading.initializing,
  );
...
export function useAuthPermissions() {
  ...
  const canAssignTaskToFn = useCallback(
  ...
  const canViewResourceFn = useCallback(
