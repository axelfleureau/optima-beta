...
-   **Production Bug Fixes (November 2025)**: Comprehensive fix di bug critici per stabilità production: (1) Stripe payment context refactor (SecurePaymentContext → MilestonePaymentContext) per supportare quote approval senza payment record preesistente, con fix TypeScript subscription fields, (2) Quote client name hydration da Firestore elimina "N/A" nei preventivi platform clients con defensive fallback, (3) AI resilience improvements con retry logic (3 attempt, exponential backoff), timeout enforcement (30s quote/caption, 60s DALL-E), structured logging, messaggi user-friendly in italiano per quote generation e image generation failures, (4) UI/UX refinements: sidebar logo 24px no overlap, workspace single-scroll pattern (horizontal main + vertical columns), task card button containment (h-7, flex-shrink-0), conditional quick-actions rendering per contentType (post/video only).
