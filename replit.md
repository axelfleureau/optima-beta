...
- **Stripe Connect Multi-Tenant Payment Architecture (October 2025)**: Three-tier payment hierarchy: Righello (platform) ← Tenant (agency, Connected Account) ← Client (end customer). **PHASE 5A Foundation Complete**: User schema extended with `stripeConnectedAccountId`, `stripeAccountStatus` ('pending' | 'active' | 'restricted' | 'rejected'), and `stripeOnboardingComplete` boolean. Client schema extended with `stripeCustomerId`, `defaultPaymentMethodId`, and `paymentMethodType` ('card' | 'sepa_debit' | 'bank_transfer'). API route `/api/stripe/connect/onboard` handles tenant onboarding with intelligent re-entry flow: retrieves existing accounts, checks `charges_enabled` + `payouts_enabled` for completion status, generates AccountLink for incomplete onboarding (enables resume), persists completion status to Firestore, gracefully handles account retrieval failures by creating new accounts. Webhook handler extended for `account.updated` events, mirroring Stripe account status back to Firestore for state consistency. Security: admin/direzione role restriction, STRIPE rate limiting profile, null safety checks throughout. Production-ready foundation for PHASE 5B (client payment method setup) and PHASE 5C (quote auto-payment via webhook).
