export type StrategicMcpConnectorStatus =
  "enabled" | "partial" | "missing" | "external";
...
const CONNECTORS: ConnectorSpec[] = [
  {
    ...
    category: "email",
    purpose:
      "Email transazionali, riepiloghi rapportini, inviti team e notifiche operative.",
    ...
    healthCheck:
      "Invio dry-run o email di test verso un indirizzo interno autorizzato, con log di esito.",
    notes:
      "Usato da Optima per invii ordinati e tracciabili, non come canale generico non governato.",
  },
  {
    id: "telegram",
    label: "Telegram",
    category: "messaging",
    purpose:
      "Canale conversazionale Optima: riceve indicazioni operative e risponde usando AI Assistant, memoria e grafo aziendale.",
    graphUse: [
      "conversazioni",
      "assistant_memories",
      "task",
    ...
    healthCheck:
      "Messaggio test da utente autorizzato, risposta asincrona e riga audit nella conversazione.",
    notes:
      "Il bot non bypassa Optima: ogni messaggio viene associato a un membro autorizzato e salvato nella cronologia AI Assistant.",
  },
  {
    id: "codex",
    label: "Codex Runner",
    category: "ai",
    purpose:
      "Codex CLI come cervello operativo di Optima: patch, report, analisi e task update dal runner autorizzato.",
    ...
    optionalEnv: ["CODEX_CLI_BIN", "CODEX_SANDBOX_MODE"],
    authMethod: "runner_env",
    setupSteps: [
      "1) Sul runner/VPS crea una home Codex separata, per esempio `CODEX_HOME=/root/.codex-chatgpt`.",
      "2) Fai backup dell'auth esistente senza stamparlo: `mkdir -p ~/.codex/auth-backups && cp ~/.codex/auth.json ~/.codex/auth-backups/auth-$(date -u +%Y%m%dT%H%M%SZ).json` se il file esiste.",
      "3) Esegui login OAuth device sul runner: `CODEX_HOME=/root/.codex-chatgpt codex logout || true` poi `CODEX_HOME=/root/.codex-chatgpt codex login --device-auth`.",
      "4) Apri l'URL mostrato dal CLI, inserisci il codice one-time e lascia la sessione SSH aperta fino a login completato.",
      "5) Verifica con prompt innocuo: `printf 'Rispondi solo con: OK-CODEX-OAUTH\\n' | CODEX_HOME=/root/.codex-chatgpt codex exec --sandbox workspace-write --skip-git-repo-check -`.",
      "6) Configura Optima runner: `AGENT_RUNNER_ENABLED=true`, `CODEX_HOME=/root/.codex-chatgpt` e `AGENT_RUNNER_API_KEY` come token interno Optima-runner.",
    ],
    healthCheck:
      "Heartbeat recente, `codex --version`, login OAuth/device-auth valido, prompt dry-run OK-CODEX-OAUTH, claim job dry-run, artefatto prodotto e stato review aggiornato.",
    notes:
      "Codex non usa la callback OAuth web di Optima: il login OAuth avviene nel Codex CLI sul runner tramite device-auth. API key OpenAI/API billing solo come fallback esplicito, non percorso principale. Non stampare mai auth.json, token o env secret.",
  },
  {
    id: "browser",
    label: "Browser MCP",
    category: "browser",
    purpose:
      "Automazione browser controllata per strumenti web senza API conveniente o affidabile: login utente nel browser isolato, ricerche operative, QA visuale, backoffice e siti autorizzati.",
    graphUse: [
      "browser_sessions",
      "agent_jobs",
      "audit",
    ...
    optionalEnv: [
      "BROWSER_PROFILE_SECRET_REF",
      "BROWSER_ALLOWED_ORIGINS",
      "BROWSER_HEADLESS",
      "BROWSER_RECORDING_BUCKET",
    ],
    ...
    healthCheck:
      "Apre una pagina allowlist in Chromium, verifica che il profilo/sessione sia controllabile, produce screenshot redatto e registra audit senza eseguire azioni irreversibili.",
    notes:
      "Questo e il ponte per l'agente mini-dipendente: puo usare interfacce web autorizzate quando non esiste API conveniente, ma resta fragile e governato da allowlist, review e sessioni isolate.",
  },
  {
    id: "cloudinary",
    label: "Cloudinary",
    category: "media",
    purpose:
      "Gestione asset visuali, trasformazioni immagini/video e libreria media collegata ai clienti.",
    ...
    healthCheck:
      "Lookup account, upload o trasformazione test su asset non sensibile e salvataggio source_id.",
    notes:
      "Da usare come nodo media del grafo, con asset collegati a clienti, campagne e deliverable.",
  },
  {
    ...
    category: "local_seo",
    purpose:
      "Gestione schede locali cliente: orari, chiusure straordinarie, servizi, post locali, foto e QA informazioni pubbliche.",
    graphUse: [
      "clienti",
      "sedi",
      "local_seo",
    ...
    healthCheck:
      "OAuth installato, lista location leggibile, lettura orari corrente e simulazione update senza pubblicazione.",
    notes:
  ...
  },
  {
    ...
    category: "calendar",
    purpose:
      "Sincronizzare scadenze, riunioni, finestre editoriali e milestone cliente con calendario e task Optima.",
    graphUse: [
      "calendario_editoriale",
      "task",
      "progetti",
    ...
    healthCheck:
      "Lista calendari allowlist, creazione evento test in calendario sandbox e cancellazione controllata.",
    notes:
  ...
  },
  {
    ...
    category: "social",
    purpose:
      "Pubblicazione e insight Facebook/Instagram collegati al calendario editoriale, con flusso approvazione cliente.",
    graphUse: [
      "calendario_editoriale",
      "asset",
      "clienti",
    ...
    healthCheck:
      "OAuth valido, lettura pagine/IG account allowlist, creazione draft interno e dry-run pubblicazione non distruttivo.",
    notes:
  ...
  },
  {
    ...
    category: "social",
    purpose:
      "Pubblicazione e reportistica LinkedIn per pagine aziendali cliente collegate a campagne B2B e calendario editoriale.",
    graphUse: [
      "calendario_editoriale",
      "clienti",
      "campagne",
    ...
    healthCheck:
      "OAuth valido, lettura organizzazioni allowlist e dry-run creazione contenuto non pubblicato.",
    notes:
      "Utile per clienti B2B e contenuti corporate; non deve pubblicare direttamente da task non approvate.",
  },
  {
    id: "google-drive",
    label: "Google Drive",
    category: "cloud",
    purpose:
      "Archiviazione e reperimento deliverable cliente, materiali approvati, allegati task e pacchetti finali di progetto.",
    graphUse: [
      "deliverable",
      "task",
      "progetti",
    ...
    healthCheck:
      "Lettura cartella sandbox, upload file test redatto, recupero metadata e cancellazione controllata.",
    notes:
  ...
  },
  {
    ...
    category: "code",
    purpose:
      "Repository, branch, pull request, commit, audit tecnico e collegamento codice-progetto.",
    ...
    healthCheck:
      "Lettura repository allowlist via user autenticato, dry-run branch/PR non distruttivo e audit owner-scoped.",
    notes:
      "1-click OAuth Device Flow (RFC 8628), stile npx wrangler login. Per App server-to-server mantenere GITHUB_TOKEN.",
  },
  {
    id: "notion",
    label: "Notion",
    category: "cloud",
    purpose:
      "Knowledge base e database storici Righello: clienti, lavori, task, portali e documenti operativi da indicizzare in Optima.",
    graphUse: [
      "clienti",
      "task",
      "progetti",
    ...
    healthCheck:
      "OAuth installato o secret_ref fallback valido, query allowlist su database atteso, conteggio record e report import redatto.",
    notes:
      "Import solo da allowlist: RIG_CLIENTI, RIG_WORK e preventivi prima di tutto. Escludere sempre credenziali, accessi, token, file segreti e dump integrali.",
  },
  {
    id: "cloudflare",
    label: "Cloudflare",
    category: "cloud",
    purpose:
      "Runtime Optima, D1, R2, Workers, secret, cron, deploy e osservabilita.",
    ...
    healthCheck:
      "Ping Worker, query D1 read-only, accesso R2 artifact e deploy dry-run o preview.",
    notes:
      "1-click OAuth Device Flow (RFC 8628), stile npx wrangler login. Il client_id OAuth pubblico deve solo essere configurato in Cloudflare.",
  },
  {
    id: "vercel",
    label: "Vercel",
    category: "cloud",
    purpose:
      "Deploy e diagnostica per progetti web che restano o nascono su Vercel.",
    ...
    healthCheck:
      "Lista deployment del progetto allowlist e lettura log di build recente.",
    notes:
      "Resta un connettore importante per progetti clienti e diagnostica, anche se Optima gira su Cloudflare.",
  },
  {
    id: "hostinger",
    label: "Hostinger VPS",
    category: "hosting",
    purpose:
      "VPS gestito per runner persistente e servizi che devono restare svegli senza pagina browser aperta.",
    ...
    healthCheck:
      "Check Tailscale/SSH read-only, spazio disco, servizio runner e heartbeat recente.",
    notes:
      "Il VPS deve essere osservabile e gestito via servizio, non tramite sessione browser fragile.",
  },
];
...
export function formatStrategicMcpConnectors(
  connectors = getStrategicMcpConnectors(),
) {
  return connectors
    .map((connector) => {
      const required = connector.requiredEnv.length
