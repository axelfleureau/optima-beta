...
const statusConfig = {
  ...
  inactive: {
    label: "Inattivo",
    color:
      "bg-slate-100 text-slate-800 dark:bg-slate-700/70 dark:text-slate-200",
  },
  prospect: {
    label: "Prospect",
...
export default function ClientiPage() {
  ...
  const canViewEconomics = clients.some(
    (client) => client.canViewEconomics === true,
  );
  ...
  const stats = {
    ...
    totalValue: clients.reduce(
      (sum, client) => sum + (client.totalValue || 0),
      0,
    ),
  };
          ...
          <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
            <div className="space-y-1 md:space-y-2">
              ...
              <p className="text-slate-400 text-sm md:text-lg">
                Gestisci i tuoi clienti e prospect
              </p>
            </div>
            <Button
