...
const pageClass =
  "h-[calc(100svh-73px)] w-full overflow-y-auto overflow-x-hidden overscroll-contain bg-[#090f1d] text-slate-100 [-webkit-overflow-scrolling:touch] [touch-action:pan-y] md:h-auto md:min-h-screen md:overflow-visible"
...
function pressureLabel(days: number | null) {
  if (days === null) return { label: "Senza scadenza", className: "border-slate-500/30 bg-slate-500/10 text-slate-300" }
  if (days < 0) return { label: "Scaduto", className: "border-red-400/30 bg-red-500/15 text-red-200" }
  if (days <= 3) return { label: "Al piu presto", className: "border-rose-400/30 bg-rose-500/15 text-rose-100" }
  if (days <= 7) return { label: "Al piu tardi", className: "border-amber-400/30 bg-amber-500/15 text-amber-100" }
  return { label: "Sotto controllo", className: "border-emerald-400/30 bg-emerald-500/15 text-emerald-100" }
}
...
function personStatus(status: string) {
  if (status === "overload") return { label: "Sovraccarico", className: "bg-red-500/15 text-red-100 border-red-400/25" }
  if (status === "underload") return { label: "Carico basso", className: "bg-sky-500/15 text-sky-100 border-sky-400/25" }
  return { label: "Bilanciato", className: "bg-emerald-500/15 text-emerald-100 border-emerald-400/25" }
}

function ProgressBar({ value, tone = "pink" }: { value: number; tone?: "pink" | "green" | "amber" | "blue" }) {
  const color = {
...
export default function ManagementPage() {
  ...
  const load = useCallback(async (mode: "initial" | "refresh" = "refresh") => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || "Errore nel caricamento controllo aziendale")
      setData(payload)
      setLastUpdatedAt(new Date())
    } catch (err) {
      setError(err instanceof Error ? err.message : "Errore nel caricamento controllo aziendale")
    } finally {
  ...
  const summaryCards = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        icon: Target,
        tone: data.summary.overdueTasks > 0 ? "text-red-300" : "text-emerald-300",
      },
      {
        label: "Ore settimana",
  ...
  if (loading) {
    return (
      <div className={pageClass}>
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-6 py-8 text-slate-300">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 md:px-6 md:py-8">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-black tracking-normal text-white md:text-4xl">Controllo Aziendale</h1>
          ...
          <>
                  ...
                  <div key={card.label} className={cn(panelClass, "p-5")}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className={cn("mt-2 text-xs", mutedText)}>{card.detail}</p>
                      </div>
            ...
            <section className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
              <div className={cn(panelClass, "p-5")}>
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-white">Segnali da presidiare</h2>
                    <p className="mt-1 text-sm text-slate-400">Pattern operativi su ritardi, saturazione e rapporto tra ore lavorate e presenza netta.</p>
                  </div>
                  <Activity className="h-5 w-5 text-righello-pink" />
                </div>

                {data.signals.length === 0 ? (
                  <div className={cn(insetPanelClass, "flex items-center gap-3 p-4 text-sm text-emerald-100")}>
                  ...
                  <div className="space-y-3">
                    {data.signals.map((signal) => (
                      <div key={signal.id} className={cn(insetPanelClass, "p-4")}>
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-sm font-bold text-white">{signal.subject}</span>
                            </div>
              ...
              <div className={cn(panelClass, "p-5")}>
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-white">Task in ritardo</h2>
                    <p className="mt-1 text-sm text-slate-400">Lista breve per decidere dove intervenire prima.</p>
                  </div>
                  <Clock3 className="h-5 w-5 text-amber-200" />
                </div>
                <div className="space-y-3">
                  {data.overdueTasks.length === 0 ? (
                    <div className={cn(insetPanelClass, "p-4 text-sm text-slate-300")}>Nessun task scaduto.</div>
                  ) : (
                    data.overdueTasks.map((task) => (
                      <Link
                        key={task.id}
                        href={`/workspace?taskId=${task.id}&action=view`}
                        className={cn(insetPanelClass, "block p-4 transition hover:border-righello-pink/40 hover:bg-righello-pink/10")}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="font-bold text-white">{task.title}</p>
                            <p className="mt-1 text-xs text-slate-400">
                              {task.clientName || "Senza cliente"} · {task.assignee}
                            </p>
                          </div>
                          <ArrowUpRight className="h-4 w-4 shrink-0 text-slate-400" />
                        </div>
                        <p className="mt-3 text-xs text-red-200">Scadenza {formatDate(task.dueAt)}</p>
                      </Link>
                    ))
                  )}
                </div>
                        ...
                        <tr key={project.id} className="bg-[#0e1625]">
                          <td className="rounded-l-lg border-y border-l border-white/10 px-3 py-4">
                            <div className="flex items-center gap-3">
                              <span className={cn("h-2.5 w-2.5 rounded-full", healthClass(project.health))} />
                              <div>
                                <p className="font-bold text-white">{project.name}</p>
                                <p className="mt-1 text-xs text-slate-400">{project.clientName || "Progetto interno"}</p>
                              </div>
                            </div>
                          </td>
                          <td className="border-y border-white/10 px-3 py-4">
                            <Badge className={cn("border", pressure.className)}>{pressure.label}</Badge>
                            <p className="mt-2 text-xs text-slate-400">{formatDate(project.dueAt)}</p>
                          </td>
                          <td className="border-y border-white/10 px-3 py-4">
                            <div className="flex items-center justify-between gap-3 text-xs text-slate-300">
                              <span>{project.completedTasks}/{project.tasksCount} task</span>
            ...
            <section className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
              <div className={cn(panelClass, "p-5")}>
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-white">Monitoraggio personale</h2>
                    <p className="mt-1 text-sm text-slate-400">Carico, presenza e approcci problematici rilevati dai dati operativi.</p>
                  </div>
                ...
                <div className="space-y-3">
                  {data.people.map((person) => {
                    ...
                    return (
                      <div key={person.id} className={cn(insetPanelClass, "p-4")}>
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 truncate text-xs text-slate-400">{person.email}</p>
                          </div>
                          <div className="grid min-w-[360px] grid-cols-3 gap-4 text-sm max-md:min-w-0 max-md:grid-cols-1">
                            <div>
                              <p className="text-xs text-slate-500">Carico attività</p>
                              <p className="mt-1 font-bold text-white">{person.utilization}%</p>
                              <ProgressBar value={person.utilization} tone={person.status === "overload" ? "amber" : "blue"} />
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="mt-1 font-bold text-white">
                                {person.openTasks} aperti · {person.overdueTasks} ritardi
                              </p>
                            </div>
                          </div>
                        </div>
              ...
              <div className={cn(panelClass, "p-5")}>
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-white">Ore recenti</h2>
                    <p className="mt-1 text-sm text-slate-400">Tracciamento operativo collegato a progetti e task.</p>
                  </div>
                  <LineChart className="h-5 w-5 text-emerald-300" />
                </div>
                <div className="space-y-3">
                  {data.recentActivity.length === 0 ? (
                    <div className={cn(insetPanelClass, "p-4 text-sm text-slate-300")}>Nessuna ora registrata.</div>
                  ) : (
                    data.recentActivity.map((entry) => (
                      <div key={entry.id} className={cn(insetPanelClass, "p-4")}>
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="font-bold text-white">{entry.memberName}</p>
                            <p className="mt-1 text-sm text-slate-300">{entry.taskTitle || entry.projectName || entry.note || "Attivita"}</p>
                          </div>
                          <Badge className="border border-sky-400/25 bg-sky-500/15 text-sky-100">
                            {formatHours(entry.minutes / 60)}
                          </Badge>
                        </div>
                        <p className="mt-3 text-xs text-slate-500">{formatDate(entry.date)}</p>
                      </div>
                    ))
                  )}
                </div>
