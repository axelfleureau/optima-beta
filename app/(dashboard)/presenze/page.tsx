...
type PresenceMutationAction =
  "check-in" | "check-out" | "undo-check-out" | "absence";
...
type PersonPresence = {
  ...
  availability: {
    status:
      "asap" | "today" | "later" | "protected" | "not-available" | "unknown";
    label: string;
    detail: string;
  };
};
  ...
  calendar: {
    ...
    people: Array<{
      ...
      days: Array<{
        ...
        productivitySignal?:
          "none" | "low" | "operational" | "focus" | "long-focus" | "sprint";
        intensity: number;
        minutesLate: number;
        minutesEarly: number;
...
function sortByOperationalRole<
  T extends { name: string; role?: string | null; email?: string | null },
>(people: T[]) {
...
function presenceSignalLabel(person: PersonPresence) {
  if (person.presenceSignal === "late")
    return `Entrata +${formatMinutes(person.minutesLate)}`;
  if (person.presenceSignal === "early-exit")
    return `Uscita -${formatMinutes(person.minutesEarly)}`;
  return "";
}

function dayTimeSignalLabel(day: CalendarPersonDay) {
  if (day.signal === "late")
    return `Entrata +${formatMinutes(day.minutesLate)}`;
  if (day.signal === "early-exit")
    return `Uscita -${formatMinutes(day.minutesEarly)}`;
  return "";
}
...
function dayTimeSignalMeta(day: CalendarPersonDay) {
  if (day.signal === "late") {
    return {
      ...
      cellTone:
        "ring-1 ring-amber-300/70 shadow-[0_0_18px_rgba(251,191,36,0.2)]",
    };
  }
  ...
  if (day.signal === "early-exit") {
    return {
      ...
      cellTone:
        "ring-1 ring-cyan-300/65 shadow-[0_0_18px_rgba(34,211,238,0.18)]",
    };
  }
...
function calendarCellClass(
  ...
  if (day.status === "absent")
    return "border-red-300/75 bg-red-500/75 text-white shadow-[0_0_20px_rgba(248,113,113,0.22)]";
  if (day.status === "holiday")
    return "border-slate-300/30 bg-slate-400/10 text-slate-300 opacity-75 [background-image:repeating-linear-gradient(135deg,rgba(148,163,184,0.11)_0,rgba(148,163,184,0.11)_6px,transparent_6px,transparent_12px)]";
  if (day.status === "missing" && day.intensity === 0)
    return "border-white/10 bg-white/[0.055] text-slate-500";
  if (day.missingDurationTaskCount > 0)
    return "border-amber-300/70 bg-amber-300/20 text-amber-50 shadow-[0_0_18px_rgba(251,191,36,0.18)]";
  if (
    (day.status === "present" || day.status === "closed") &&
    day.intensity === 0
  ) {
    return "border-blue-300/45 bg-blue-400/18 text-blue-50 shadow-[0_0_16px_rgba(96,165,250,0.12)]";
  }
  if (day.productivitySignal === "long-focus") {
    return "border-violet-300/55 bg-violet-400/42 text-violet-50 shadow-[0_0_18px_rgba(167,139,250,0.18)]";
  }
...
function calendarCellSignal(
  ...
  if (day.missingDurationTaskCount > 0) {
    return {
      label: "Durate mancanti",
      short: String(day.missingDurationTaskCount),
      Icon: AlertCircle,
    };
  }
  if (
  ...
  if (day.productivitySignal === "long-focus") {
    return {
      label: "Task lunga",
      short: day.taskCount > 0 ? String(day.taskCount) : "1",
      Icon: Clock,
    };
  }
  if (day.intensity >= 4)
    return {
      label: "Sprint",
      short: day.taskCount > 0 ? String(day.taskCount) : "MAX",
      Icon: Flame,
    };
  if (day.intensity >= 3)
    return {
      label: "Focus",
      short: day.taskCount > 0 ? String(day.taskCount) : "F",
      Icon: Activity,
    };
  if (day.intensity >= 2)
    return {
      label: "Operativo",
      short: day.taskCount > 0 ? String(day.taskCount) : "ON",
      Icon: Activity,
    };
  if (day.intensity >= 1)
    return {
      label: "Bassa attività",
      short: day.taskCount > 0 ? String(day.taskCount) : "LOW",
      Icon: UserCheck,
    };
...
function calendarDayTitle(
  ...
  const parts = [
    ...
    `Valore operativo: ${formatMinutes(day.activityMinutes)}`,
    `Task collegate: ${day.taskCount}`,
    `Granularità lavoro: ${day.throughputScore || Math.max(day.taskCount, day.entryCount, day.completedTaskCount || 0)}`,
  ];
  if (day.completedTaskCount > 0)
    parts.push(`Task completate/importate: ${day.completedTaskCount}`);
  if (day.missingDurationTaskCount > 0)
    parts.push(`Task senza durata: ${day.missingDurationTaskCount}`);
  ...
  if (day.signal === "late")
    parts.push(`Ritardo: ${formatMinutes(day.minutesLate)}`);
  if (day.signal === "early-exit")
    parts.push(`Uscita anticipata: ${formatMinutes(day.minutesEarly)}`);
  if (day.absenceReason) parts.push(`Motivo: ${day.absenceReason}`);
  return parts.join("\n");
}

function personMonthStats(
  person: PresencePayload["calendar"]["people"][number],
) {
  const stats = person.days.reduce(
    (acc, day) => {
      if (day.status === "present" || day.status === "closed")
        acc.presenceDays += 1;
...
function getCalendarDayRecords(people: CalendarPerson[], date: string) {
  return people
    ...
    .filter(
      (record): record is { person: CalendarPerson; day: CalendarPersonDay } =>
        Boolean(record.day),
    );
}
...
function calendarDayTone(
  ...
  const hasPresence = records.some(
    ({ day }) => day.status === "present" || day.status === "closed",
  );
...
function attendanceEventLabel(day: CalendarPersonDay) {
  if (day.status === "absent")
    return day.absenceReason ? `Assente: ${day.absenceReason}` : "Assente";
  if (day.status === "holiday") return day.absenceReason || "Festivo";
  if (day.signal === "late")
    return `Ritardo +${formatMinutes(day.minutesLate)}`;
  if (day.signal === "early-exit")
    return `Uscita -${formatMinutes(day.minutesEarly)}`;
...
export default function PresenzePage() {
  ...
  const [checkInTimeDirty, setCheckInTimeDirty] = useState(false);
  const [checkOutTimeDirty, setCheckOutTimeDirty] = useState(false);
    ...
    async (mode: "initial" | "refresh" = "refresh") => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/time-tracking/presence?${params.toString()}`,
          {
            cache: "no-store",
            headers: {
        ...
        if (!response.ok)
          throw new Error(data.error || "Errore nel caricamento presenze");
        setPayload(data);
      } catch (loadError) {
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Errore nel caricamento presenze",
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const presentPeople = useMemo(
    () => payload?.people.filter((person) => person.status === "present") || [],
    [payload],
  );
  ...
  const availabilityPeople = useMemo(
    () =>
      [...(payload?.people || [])].sort((a, b) => {
        ...
        return (
          (order[a.availability?.status] ?? 9) -
          (order[b.availability?.status] ?? 9)
        );
      }),
    [payload],
  );
  ...
  useEffect(() => {
    if (!checkInTimeDirty) {
      setCheckInTime(
        timeInputValue(
          self?.checkInAt,
          self?.workStartTime || defaultWorkStartTime,
        ),
      );
    }
    if (!checkOutTimeDirty) {
      setCheckOutTime(
        timeInputValue(
          self?.checkOutAt,
          self?.expectedCheckOutTime || currentTime(),
        ),
      );
    }
  }, [
    ...
    self?.checkInAt,
    self?.checkOutAt,
    self?.expectedCheckOutTime,
    self?.workStartTime,
  ]);

  useEffect(() => {
    setCheckInTimeDirty(false);
    setCheckOutTimeDirty(false);
  }, [date]);
  ...
  const mutateSelf = async (action: PresenceMutationAction, time?: string) => {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok)
      throw new Error(data.error || "Errore aggiornamento presenza");
    setCheckInTimeDirty(false);
    setCheckOutTimeDirty(false);
    await load("refresh");
  };
  ...
  const submitQuickTask = async () => {
    ...
    if (!title) {
      toast.error(
        "Scrivi prima cosa è stato fatto: diventa il titolo della task.",
      );
      return;
    }
      ...
      <div className="optima-ops-container optima-ops-stack relative overflow-x-clip">
        <header className="optima-ops-header">
          <div className="min-w-0">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400 md:text-base">
              Vista rapida per verificare chi è in ufficio, chi ha segnato
              assenze e come sta andando la copertura delle ore nette. La
              giornata parte di norma alle 09:00 e termina alle 18:00, con
              entrata reale anticipabile o posticipabile.
            </p>
          </div>
        ...
        <section className="grid min-w-0 gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            ...
            <div className="space-y-5 p-5">
              <div className="grid min-w-0 gap-3 sm:grid-cols-2">
                <div className="min-w-0 overflow-hidden rounded-[8px] border border-white/10 bg-white/[0.04] p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Prevista {self?.workStartTime || defaultWorkStartTime}
                  </p>
                  <Input
                    type="time"
                    value={checkInTime}
                    disabled={!payload?.isManager}
                    onChange={(event) => {
                ...
                <div className="min-w-0 overflow-hidden rounded-[8px] border border-white/10 bg-white/[0.04] p-4">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Indicativa {self?.expectedCheckOutTime || "17:00"}
                  </p>
                  <Input
                    type="time"
                    value={checkOutTime}
                    ...
                    <p className="mt-1 text-sm text-slate-300">
                      {(self?.taskCount ?? 0) > 0 ? (
                        ...
                        <>
                          ...
                          — il rapportino della giornata sarà vuoto se esci
                          adesso.
                        </>
                      )}
                    </p>
                ...
                <Button
                  ...
                  onClick={() =>
                    mutateSelf(
                      "check-in",
                      payload?.isManager &&
                        (checkInTimeDirty || date !== today())
                        ? checkInTime
                        : undefined,
                    )
                      .then(() => toast.success("Check-in registrato"))
                ...
                <Button
                  ...
                  onClick={() =>
                    mutateSelf(
                      "check-out",
                      payload?.isManager &&
                        (checkOutTimeDirty || date !== today())
                        ? checkOutTime
                        : undefined,
                    )
                      .then(() => toast.success("Check-out registrato"))
                      .catch((err) => toast.error(err.message))
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    mutateSelf("undo-check-out")
                      .then(() => toast.success("Check-out annullato"))
                      .catch((err) => toast.error(err.message))
                  }
                >
              ...
              <div className="rounded-[8px] border border-white/10 bg-black/20 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm text-slate-400">
                      Presenza netta registrata
                    </p>
                    <p className="mt-1 text-2xl font-black text-white">
                      {formatMinutes(self?.presenceMinutes || 0)} /{" "}
                      {formatMinutes(self?.expectedOfficeMinutes || 420)}
                    </p>
                  </div>
                ...
                <Progress
                  ...
                  value={(self?.coverageRatio || 0) * 100}
                />
                <p className="mt-3 text-xs leading-5 text-slate-500">
                  Dato di presenza:{" "}
                  {formatMinutes(self?.dailyCapacityMinutes || 480)} lorde, meno{" "}
                  {formatMinutes(self?.lunchBreakMinutes || 60)} di pausa
                  pranzo. Serve per controllo orario; il valore operativo resta
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <SummaryCard
              label="Presenti ora"
              value={payload?.summary.present || 0}
              tone="green"
            />
            <SummaryCard
              label="Usciti"
              value={payload?.summary.closed || 0}
              tone="cyan"
            />
            <SummaryCard
              label="Non segnati"
              value={payload?.summary.missing || 0}
              tone="amber"
            />
            <SummaryCard
              label="Assenti"
              value={payload?.summary.absent || 0}
              tone="red"
            />
            <div className={cn(panelClass, "p-4 sm:col-span-2 lg:col-span-4")}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm text-slate-400">
                    Valore operativo team
                  </p>
                  <p className="mt-1 text-3xl font-black text-cyan-100">
                    {formatMinutes(payload?.summary.activityMinutes || 0)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-slate-400">Presenza monitorata</p>
                  <p className="mt-1 text-2xl font-black text-slate-200">
                    {formatMinutes(payload?.summary.presenceMinutes || 0)}
                  </p>
                </div>
              </div>
            </div>
        ...
        {payload?.isManager ? (
              ...
              <div className="grid gap-3 border-b border-white/10 p-5">
                <div>
                  <h3 className="text-lg font-black text-white">
                    Prossime finestre operative
                  </h3>
                  <p className="mt-1 text-sm leading-6 text-slate-400">
                    Lettura rapida per capire chi può prendere qualcosa al più
                    presto e chi va lasciato sul carico già pianificato.
                  </p>
                </div>
          ...
          <section className={cn(panelClass, "p-5")}>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              Questa vista tiene separato il concetto di presenza dal
              rapportino: prima segni entrata/uscita, poi colleghi le ore alle
              attività quando serve.
            </p>
          </section>
        )}
      </div>
...
function PresenceCalendarHeatmap({
  ...
  const [activeSignal, setActiveSignal] =
    useState<HeatmapSignalSelection | null>(null);
  const [activeCell, setActiveCell] = useState<HeatmapCellSelection | null>(
    null,
  );
  const [activeDay, setActiveDay] = useState<HeatmapDaySelection | null>(null);

  const orderedCalendarPeople = useMemo(
    () => sortByOperationalRole(calendar.people),
    [calendar.people],
  );
  ...
  useEffect(() => {
    if (
      personFilter !== "all" &&
      !orderedCalendarPeople.some((person) => person.id === personFilter)
    ) {
      setPersonFilter("all");
    }
  ...
  useEffect(() => {
    setActiveSignal((current) => {
      ...
      const person = visiblePeople.find(
        (item) => item.id === current.person.id,
      );
  ...
  const activeHeatmapSelection = useMemo(() => {
    ...
    return meaningfulRecord
      ? { person: meaningfulRecord.person, day: meaningfulRecord.day }
      : null;
        ...
        <>
                    ...
                    <div className="flex min-w-0 items-start justify-between gap-3">
                      ...
                      <div className="shrink-0 rounded-[8px] border border-white/10 bg-white/[0.04] px-3 py-2 text-right">
                        ...
                        <p className="mt-1 text-sm font-black text-cyan-100">
                          {formatMinutes(stats.activityMinutes)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid auto-cols-[58px] grid-flow-col gap-2">
                      {person.days.map((day) => (
                        <CalendarHeatmapCell
                          ...
                          active={
                            activeCell?.person.id === person.id &&
                            activeCell.day.date === day.date
                          }
                          compact={false}
            ...
            <div className="w-max min-w-full">
              <div className="grid items-end gap-1" style={heatmapGridStyle}>
                ...
                {calendar.days.map((day) => (
                  ...
                  >
                    <p className="text-[13px] font-black leading-none text-slate-100">
                      {formatDayNumber(day)}
                    </p>
                    <p className="mt-1 text-[10px] font-bold uppercase leading-none text-slate-500">
                    ...
                    >
                      <p
                        className={cn(
                          ...
                          compactPeopleColumn
                            ? "max-w-[96px] text-xs leading-tight"
                            : "text-sm",
                        )}
                      >
                        {compactPeopleColumn
                          ? compactPersonLabel(person.name)
                          : person.name}
                      </p>
                      {!compactPeopleColumn && (
          ...
          <div className="flex flex-wrap items-center gap-4 border-t border-white/10 px-5 py-4 text-xs text-slate-300">
            ...
            <span className="inline-flex items-center gap-2">
              ...
              <span>
                da consuntivare · bassa attività · operativo · focus · sprint
              </span>
            </span>
            ...
            <span className="inline-flex items-center gap-1.5">
              <span className="h-4 w-4 rounded-[5px] border border-white/10 bg-white/[0.055]" />{" "}
              non segnato
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="h-4 w-4 rounded-[5px] border border-red-300/70 bg-red-500/70" />{" "}
              assenza
            </span>
            <span className="inline-flex items-center gap-1.5">
            ...
            <span className="inline-flex items-center gap-1.5">
              <span className="h-4 w-4 rounded-[5px] outline outline-2 outline-righello-pink/70" />{" "}
              giorno selezionato
            </span>
          </div>
        </>
...
function PresenceMonthCalendar({
  ...
  const cells = useMemo(
    () => getMonthCalendarCells(calendar.days),
    [calendar.days],
  );
    ...
    <div className="p-4">
      <div className="mb-4 rounded-[8px] border border-white/10 bg-black/20 p-4">
        ...
        <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-400">
          Qui il click apre il dettaglio aggregato del giorno. Per correggere
          una singola persona usa la heatmap e seleziona la cella
          persona-giorno.
        </p>
      ...
      <div className="space-y-3 md:hidden">
        {calendar.days.map((date) => {
          ...
          const importantRecords = records.filter(
            ({ day }) => day.status === "absent" || day.signal,
          );
          const visibleRecords =
            importantRecords.length > 0 ? importantRecords : records;
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-righello-pink">
                ...
                <div className="grid grid-cols-4 gap-1 text-center text-[11px]">
                  <span className="rounded-[6px] border border-emerald-300/20 bg-emerald-300/10 px-2 py-1 text-emerald-100">
                    {summary.present} P
                  </span>
                  <span className="rounded-[6px] border border-amber-300/20 bg-amber-300/10 px-2 py-1 text-amber-100">
                    {summary.late + summary.earlyExit} !
                  </span>
                  <span className="rounded-[6px] border border-red-300/20 bg-red-500/10 px-2 py-1 text-red-100">
                    {summary.absent} A
                  </span>
                  <span className="rounded-[6px] border border-slate-300/15 bg-slate-300/10 px-2 py-1 text-slate-200">
                    {summary.holiday} F
                  </span>
                </div>
              </div>
              <div className="mt-3 grid gap-2">
                ...
                {visibleRecords.length > 6 && (
                  <p className="text-xs font-bold text-slate-400">
                    +{visibleRecords.length - 6} altri record
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="hidden md:block">
        <div className="grid grid-cols-7 gap-2 pb-2">
          {weekdayLabels.map((label) => (
            <div
              key={label}
              className="px-2 text-xs font-black uppercase tracking-[0.14em] text-slate-500"
            >
              {label}
            </div>
          ))}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-wrap justify-end gap-1 text-[10px] font-black">
                    <span className="rounded-[6px] bg-emerald-300/12 px-1.5 py-1 text-emerald-100">
                      {summary.present}
                    </span>
                    {(summary.late > 0 || summary.earlyExit > 0) && (
                      <span className="rounded-[6px] bg-amber-300/14 px-1.5 py-1 text-amber-100">
                        !
                      </span>
                    )}
                    {summary.absent > 0 && (
                      <span className="rounded-[6px] bg-red-500/16 px-1.5 py-1 text-red-100">
                        {summary.absent}
                      </span>
                    )}
                    {summary.holiday > 0 && (
                      <span className="rounded-[6px] bg-slate-300/12 px-1.5 py-1 text-slate-200">
                        {summary.holiday}F
                      </span>
                    )}
                  </div>
                ...
                <div className="mt-3 space-y-1.5">
                  {visibleRecords.slice(0, 4).map(({ person, day }) => (
                    <div
                      key={`${date}-${person.id}`}
                      className={cn(
                        "min-w-0 rounded-[6px] border px-2 py-1.5 text-[11px]",
                        attendanceEventTone(day),
                      )}
                    >
                  ...
                  {visibleRecords.length === 0 && (
                    <p className="text-xs text-slate-500">Nessun dato</p>
                  )}
                  {visibleRecords.length > 4 && (
                    <p className="text-[11px] font-bold text-slate-400">
                      +{visibleRecords.length - 4} altri
                    </p>
                  )}
                </div>
              </div>
...
function CalendarHeatmapCell({
    ...
    return (
      <button
        ...
        className={cn(
          ...
          selected &&
            "outline outline-2 outline-offset-1 outline-righello-pink/70",
      ...
      >
        {timeSignal && day.status !== "absent" && TimeSignalIcon && (
          <span
            className={cn(
              "absolute right-0.5 top-0.5 inline-flex h-4 w-4 items-center justify-center rounded-full border",
              timeSignal.chipTone,
            )}
            aria-hidden="true"
          >
        ...
        {timeSignal && day.status !== "absent" && (
          <span
            className={cn(
              "absolute bottom-0.5 left-1/2 max-w-[92%] -translate-x-1/2 rounded-[5px] border px-1 text-[8px] font-black leading-3",
              timeSignal.chipTone,
            )}
          >
            {timeSignal.short}
  ...
  return (
    ...
    >
      <span className="absolute left-1.5 top-1.5 text-[10px] font-black leading-none opacity-70">
        {formatDayNumber(day.date)}
      </span>
      {timeSignal && day.status !== "absent" && TimeSignalIcon && (
        <span
          className={cn(
            "absolute right-1 top-1 inline-flex h-6 w-6 items-center justify-center rounded-full border",
            timeSignal.chipTone,
          )}
          aria-hidden="true"
        >
      ...
      <span className="mt-1 max-w-full truncate text-[9px] font-bold uppercase leading-none opacity-80">
        {signal.label}
      </span>
      {timeSignal && day.status !== "absent" && (
        <span
          className={cn(
            "absolute bottom-1.5 inline-flex max-w-[90%] items-center gap-1 rounded-[6px] border px-1.5 py-0.5 text-[9px] font-black leading-none",
            timeSignal.chipTone,
          )}
        >
...
function HeatmapCellContextBanner({
  ...
  const [adminCheckInDirty, setAdminCheckInDirty] = useState(false);
  const [adminCheckOutDirty, setAdminCheckOutDirty] = useState(false);
  const [adminSavingAction, setAdminSavingAction] =
    useState<PresenceMutationAction | null>(null);
  const adminSelectionKeyRef = useRef("");

  useEffect(() => {
    if (!selection) {
      adminSelectionKeyRef.current = "";
      setAdminCheckInDirty(false);
      setAdminCheckOutDirty(false);
      return;
    }
    const selectionKey = `${selection.person.id}:${selection.day.date}`;
    const isNewSelection = adminSelectionKeyRef.current !== selectionKey;

    if (isNewSelection) {
      adminSelectionKeyRef.current = selectionKey;
      setAdminCheckInDirty(false);
      setAdminCheckOutDirty(false);
    ...
    if (!adminCheckInDirty)
      setAdminCheckInTime(timeInputValue(selection.day.checkInAt, "09:00"));
    if (!adminCheckOutDirty)
      setAdminCheckOutTime(timeInputValue(selection.day.checkOutAt, "18:00"));
  }, [
    adminCheckInDirty,
    adminCheckOutDirty,
    selection?.day.checkInAt,
    selection?.day.checkOutAt,
    selection?.day.date,
    selection?.person.id,
  ]);
  ...
  const mutateSelectedDay = async (
    ...
    try {
      ...
      if (action === "check-in") setAdminCheckInDirty(false);
      if (action === "check-out") setAdminCheckOutDirty(false);
      toast.success(
        action === "absence"
          ...
          <div className="relative flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <h3 className="mt-1 flex min-w-0 items-center gap-2 text-xl font-black text-white">
                ...
                >
                  {timeSignal ? (
                    <TimeSignalIcon className="h-5 w-5" />
                  ) : (
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
            ...
            <div className="rounded-[8px] border border-white/10 bg-white/[0.04] p-3">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-400">
                ...
                {day.missingDurationTaskCount > 0
                  ? ` · ${day.missingDurationTaskCount} senza durata`
                  : ""}
              </p>
            </div>
            <div className="rounded-[8px] border border-white/10 bg-white/[0.04] p-3">
            ...
            <div className="rounded-[8px] border border-righello-cyan/20 bg-righello-cyan/10 p-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-400">
                    Queste azioni modificano solo la giornata di{" "}
                    {selection.person.name}. Usa “Apri giorno” per la vista
                    aggregata del team.
                  </p>
                </div>
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto]">
                ...
                <Button
                  ...
                  onClick={() =>
                    mutateSelectedDay(
                      "check-in",
                      adminCheckInDirty || day.date !== today()
                        ? adminCheckInTime
                        : undefined,
                    )
                  }
                ...
                <Button
                  ...
                  onClick={() =>
                    mutateSelectedDay(
                      "check-out",
                      adminCheckOutDirty || day.date !== today()
                        ? adminCheckOutTime
                        : undefined,
                    )
                  }
              ...
              <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs leading-5 text-slate-400">
                  Entrata e uscita salvate qui aggiornano heatmap, calendario e
                  rapportini al prossimo refresh automatico.
                </p>
                <Button
                  type="button"
                  variant="outline"
        ...
        <div className="space-y-3 p-4">
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <p>
                Indicatore secondario: l'orario va letto insieme alla produzione
                del giorno. Output rilevato:{" "}
...
function HeatmapDayDialog({
  ...
  const taskTitles = selection.records.flatMap(
    ({ day }) => day.taskTitles || [],
  );
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-white/10 p-5">
          <div className="min-w-0">
            ...
            <h3 className="mt-1 text-2xl font-black capitalize text-white">
              ...
              >
                ...
                {day.completedTaskCount > 0 ? (
                  <p className="mt-2 text-xs opacity-75">
                    {day.completedTaskCount} task completate/importate nel
                    giorno
                    {day.missingDurationTaskCount > 0
                      ? ` · ${day.missingDurationTaskCount} senza durata`
                      : ""}
...
function PresenceColumn({
  ...
  return (
    <div
      className={cn(
        "min-w-0 p-5",
        muted && "border-t border-white/10 md:border-l md:border-t-0",
      )}
    >
    ...
    <div className="rounded-[8px] border border-white/10 bg-white/[0.035] p-4">
      ...
      {onMarkAbsent && (
        <Button
          ...
          onClick={() =>
            onMarkAbsent(person.id, person.name).catch((err) =>
          ...
          {person.nextTask ? (
            <div className="mt-2">
              <p className="break-words text-sm font-bold leading-5 text-white">
                {person.nextTask.title}
              </p>
              <p className="mt-1 text-xs leading-5 text-slate-400">
                {person.nextTask.projectName ||
                  person.nextTask.clientName ||
                  "Task operativa"}{" "}
                · {formatDueDate(person.nextTask.dueAt)}
              </p>
            </div>
          ) : (
            <p className="mt-2 text-sm text-slate-400">
              Nessuna task imminente collegata.
            </p>
          )}
        </div>
  ...
  return (
    <div className="grid gap-3 border-b border-white/10 p-4 last:border-b-0 md:grid-cols-[1.1fr_0.65fr_0.75fr_0.85fr_1.2fr_auto] md:items-center">
      ...
      <div className="text-sm text-slate-300">
        ...
        <p className="mt-1 text-xs text-slate-500">
          {person.assumedPresence ? "Direzione reperibile" : "Rif."}{" "}
          {person.workStartTime} - {person.expectedCheckOutTime}
        </p>
      </div>
      <div>
        ...
        <Progress
          ...
          value={person.coverageRatio * 100}
        />
        {person.presenceSignal && (
      ...
      <div className="min-w-0 rounded-[8px] border border-white/10 bg-white/[0.03] p-3">
        ...
        <p className="mt-1 truncate text-xs text-slate-500">
          {person.nextTask?.title || person.availability?.detail}
        </p>
      </div>
      {onMarkAbsent && (
        <Button
