...
const EVENT_TYPES = [
  {
    ...
    label: "Riunione",
    icon: Users,
    className: "border-cyan-400/35 bg-cyan-400/12 text-cyan-100",
  },
  {
    value: "shooting",
    label: "Shooting",
    icon: Camera,
    className: "border-pink-400/40 bg-pink-500/14 text-pink-100",
  },
  {
    value: "call",
    label: "Call",
    icon: Phone,
    className: "border-blue-400/35 bg-blue-500/14 text-blue-100",
  },
  {
    value: "delivery",
    label: "Delivery",
    icon: CalendarClock,
    className: "border-emerald-400/35 bg-emerald-500/14 text-emerald-100",
  },
  {
    value: "internal",
    label: "Interno",
    icon: Video,
    className: "border-violet-400/35 bg-violet-500/14 text-violet-100",
  },
  {
    value: "travel",
    label: "Trasferta",
...
export default function TeamCalendarClient() {
  ...
  const calendarCells = useMemo(() => {
    ...
    for (let index = mondayOffset; index > 0; index -= 1) {
      cells.push({
        date: new Date(cursor.getFullYear(), cursor.getMonth(), 1 - index),
        inMonth: false,
      });
    }
  ...
  const submitEvent = async () => {
    ...
    try {
      ...
      const response = await fetch(
        form.id ? `/api/team-calendar/${form.id}` : "/api/team-calendar",
        {
      ...
      await loadCalendar();
      toast({
        title: form.id ? "Evento aggiornato" : "Evento creato",
        description: "Il calendario team è stato aggiornato.",
      });
    } catch (error) {
      toast({
  ...
  const deleteEvent = async () => {
    if (
      !form.id ||
      !window.confirm("Eliminare questo evento dal calendario team?")
    )
      return;
    setIsSaving(true);
    try {
      const response = await fetch(`/api/team-calendar/${form.id}`, {
        method: "DELETE",
      });
  ...
  const copyFeed = async () => {
    ...
    toast({
      title: "Link copiato",
      description:
        "Puoi incollarlo in Calendario iCloud come calendario in abbonamento.",
    });
  };
      ...
      <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 rounded-lg border border-white/10 bg-white/[0.035] p-4 sm:flex-row sm:items-end sm:justify-between sm:p-5">
          <div>
            ...
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-righello-cyan">
              Operazioni interne
            </p>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/62">
              Pianifica shooting, call, appuntamenti, delivery e impegni del
              team. Il feed iCalendar lo puoi collegare a iCloud per averlo sul
              telefono.
              {canSeeTeam
          ...
          <div className="flex flex-wrap gap-2">
            {canSeeTeam && (
              <Button
                ...
                variant="outline"
                onClick={() =>
                  setScope((current) => (current === "team" ? "mine" : "team"))
                }
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.035]">
            <div className="flex flex-col gap-3 border-b border-white/10 p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="min-w-[180px] text-center text-lg font-black capitalize">
                  {formatMonth(cursor)}
                </h2>
                <Button
                  type="button"
            ...
            <div className="hidden lg:block">
              <div className="grid grid-cols-7 border-b border-white/10">
                {WEEKDAYS.map((day) => (
                  <div
                    key={day}
                    className="px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-white/42"
                  >
                    {day}
                  </div>
                ))}
            ...
            <div className="space-y-3 p-3 lg:hidden">
              {agenda.length === 0 && !isLoading ? (
                <div className="rounded-lg border border-dashed border-white/12 p-5 text-center text-sm text-white/50">
                  Nessun evento nel mese. Aggiungi shooting, call o
                  appuntamenti.
                </div>
              ) : (
                agenda.map((event) => (
          ...
          <aside className="space-y-4">
            <div className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
              ...
              <div className="mt-4 space-y-3">
                ...
                {agenda.length === 0 && (
                  <p className="rounded-lg border border-dashed border-white/12 p-4 text-sm text-white/50">
            ...
            <div className="rounded-lg border border-righello-cyan/20 bg-righello-cyan/10 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-white/62">
                Il primo step è un calendario in abbonamento: lo vedi su iPhone,
                Mac e Google Calendar, mentre le modifiche restano governate da
                Optima.
                {canSeeTeam
                  ? " Il feed direzione include tutto il team."
                  : " Il feed dipendente include solo i tuoi eventi."}
              </p>
            </div>
          </aside>
        </section>
        ...
        <DialogContent className="max-h-[92dvh] overflow-y-auto border-white/10 bg-[#050914] p-0 text-white sm:max-w-3xl">
          <DialogHeader className="border-b border-white/10 p-5">
            ...
            <DialogDescription className="text-white/50">
              Collega l'impegno a cliente, progetto e referente quando serve
          ...
          <div className="grid gap-4 p-5 sm:grid-cols-2">
            ...
            <label className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-3 text-sm text-white/72 sm:col-span-2">
              <input
                ...
                checked={form.allDay}
                onChange={(event) =>
                  updateForm({ allDay: event.target.checked })
                }
              />
              Tutto il giorno
            </label>
            ...
            <Field label="Luogo" className="sm:col-span-2">
              <Input
                ...
                placeholder="Studio, ufficio, indirizzo shooting..."
            ...
            <Field label="Referente">
              <NativeSelect
                ...
                onChange={(value) => updateForm({ ownerMemberId: value })}
              >
                {members.map((member) => (
        ...
        <DialogContent className="border-white/10 bg-[#050914] text-white sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/55">
              Usa questo URL come calendario in abbonamento su iPhone, Mac o
              Google Calendar. Il feed rispetta i permessi del profilo.
            </DialogDescription>
          </DialogHeader>
          <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-sm text-white/70 break-all">
            {feedUrl}
          </div>
          <ol className="space-y-2 text-sm leading-6 text-white/62">
            <li>
              1. iPhone: Impostazioni &gt; Calendario &gt; Account &gt; Aggiungi
              account &gt; Altro.
            </li>
            <li>2. Scegli “Aggiungi calendario con iscrizione”.</li>
            <li>
              3. Google Calendar: Altri calendari &gt; Da URL, oppure apri il
              link Google sotto.
            </li>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              type="button"
              onClick={copyFeed}
              className="bg-righello-cyan text-slate-950 hover:bg-righello-cyan/90"
            >
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="flex-shrink-0 rounded-full bg-white/8 px-2 py-1 text-xs text-white/56">
              {formatTimeRange(event)}
            </span>
          </div>
          {!compact && (
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-xs text-white/50">
            {event.ownerName && (
              <span className="rounded-full bg-white/7 px-2 py-0.5">
                {event.ownerName}
              </span>
            )}
            {event.clientName && (
...
function Field({
  ...
  return (
    <div className={cn("space-y-2", className)}>
      <Label className="text-xs font-bold uppercase tracking-[0.12em] text-white/42">
        {label}
      </Label>
      {children}
    </div>
