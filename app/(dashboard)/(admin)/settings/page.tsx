...
function AgenticGitHubPolicyCard() {
  ...
  async function loadPolicy() {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.error || "Policy GitHub non caricata.");
  ...
  async function savePolicy() {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(data.error || "Policy GitHub non salvata.");
  ...
  return (
    <div className="rounded-lg border border-cyan-300/20 bg-[#0b1321]/90 p-5 shadow-[0_20px_70px_rgba(34,211,238,0.08)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-semibold text-white">
                GitHub MCP owner-scoped
              </h4>
              <p className="max-w-3xl text-sm text-slate-400">
                Le repo aziendali sono accessibili al sistema agentico, ma
                commit, push e deploy con l'account GitHub di Axel restano una
                capability personale dell'owner tecnico.
              </p>
            </div>
          </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr]">
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-500">
            Utente corrente: {currentUserEmail || "non disponibile"}. Gli altri
            ruoli possono proporre/revisionare, non usare il tuo GitHub.
          </p>
        </div>

        <div className="space-y-2">
          <Label
            htmlFor="github-repo-patterns"
            className="flex items-center gap-2 text-slate-200"
          >
          ...
          />
          <p className="text-xs text-slate-500">
            Accetta pattern come `axelfleureau/*` o URL GitHub specifici. I repo
            fuori scope non generano job publish/deploy.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        <div className="flex items-center justify-between rounded-lg border border-white/10 bg-[#070d18]/80 p-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">
              Permette al job approvato di pubblicare su GitHub.
            </p>
          </div>
          <Switch
            checked={commitPushEnabled}
            onCheckedChange={setCommitPushEnabled}
            disabled={!canEdit || loading}
          />
        </div>
        <div className="flex items-center justify-between rounded-lg border border-white/10 bg-[#070d18]/80 p-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">
              Crea il job di deploy solo quando approva l'owner GitHub
              autorizzato.
            </p>
          </div>
          <Switch
            checked={deployEnabled}
            onCheckedChange={setDeployEnabled}
            disabled={!canEdit || loading}
          />
        </div>
      </div>
        ...
        <div className="mt-4">
          ...
          {!canEdit && !loading && !error && (
            <Alert className="border-amber-300/25 bg-amber-300/[0.1] text-amber-100">
              <AlertDescription>
                Vista in sola lettura: solo l'owner GitHub configurato può
                cambiare questa policy.
              </AlertDescription>
            </Alert>
          )}
        </div>
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-slate-500">
          Nessun token viene salvato qui: Optima conserva solo policy, scope,
          stato MCP e riferimento protetto.
        </p>
        <Button
          type="button"
          onClick={savePolicy}
