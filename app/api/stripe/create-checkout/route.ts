  ...
  try {
    ...
    if (userData.tenantId) {
      ...
      if (tenantDoc.exists) {
        ...
        console.error(`❌ Tenant ${userData.tenantId} not found for user ${userId}`)
        return null
      }
    } else {
...
async function getQuoteSecurely(quoteId: string, tenantId: string): Promise<Quote | null> {
  try {
    if (!adminDb) {
      console.error("❌ Firebase Admin DB not initialized")
      return null
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!authUser) {
      console.error("❌ Authentication failed - invalid or missing Firebase token")
      return NextResponse.json(
        { 
          success: false, 
          error: "Authentication required", 
          details: "Invalid or expired Firebase authentication token. Please log in again.",
    ...
    if (!userData || !userData.user || !userData.tenant) {
      console.error(`❌ User or tenant data not found for UID: ${authUser.uid}`)
      return NextResponse.json(
        { 
          success: false, 
          error: "User or tenant not found", 
          details: "Your account may not be properly configured or may be suspended. Please contact support.",
    ...
    const quote = await getQuoteSecurely(requestData.quoteId, userData.tenant.id)
    if (!quote) {
    ...
    console.log(`✅ Quote validated: ${quote.id} - ${quote.title || 'Untitled'}`)
    ...
    const checkoutResult = await stripeService.createCheckoutSession(requestData, paymentContext)
    ...
    console.log(`✅ Checkout session created: ${checkoutResult.data.checkoutSessionId}`)
    ...
    try {
      const paymentId = await createPayment({
        ...
        clientName: requestData.clientName || quote.clientName || 'Unknown Client',
