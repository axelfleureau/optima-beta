...
async function loadProject(db: any, organizationId: string, projectId: string) {
  ...
  return mapProjectRows(projectResult.results || [], memberResult.results || [])[0]
}
  ...
  try {
    ...
    if ("clientId" in body) {
      ...
      if (clientId) {
        const client = await db
          .prepare(`SELECT id FROM clients WHERE organization_id = ? AND id = ? LIMIT 1`)
    ...
    if (assignments.length > 0) {
      ...
      const result = await db
        .prepare(`UPDATE projects SET ${assignments.join(", ")} WHERE id = ? AND organization_id = ?`)
        .bind(...values)
        .run()
    ...
    if (memberIds) {
      await db
        .prepare(`DELETE FROM project_members WHERE organization_id = ? AND project_id = ?`)
      ...
      for (const memberId of safeMemberIds) {
        await db
          ...
          .bind(id, memberId === principal.memberId ? "owner" : "member", principal.organizationId, memberId)
          .run()
      }
    }
