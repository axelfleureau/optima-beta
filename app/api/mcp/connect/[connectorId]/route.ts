...
type AuthMethod = "oauth_pkce" | "device_flow" | "external_oauth" | "browser_session_oauth" | "github_app" | "runner_env" | "api_key_secret" | "service_account"
...
const DEVICE_FLOW_DEFAULTS: Record<string, {
...
export async function POST(request: NextRequest, context: { params: Promise<{ connectorId: string }> }) {
  try {
    ...
    return Response.json({ error: error?.message ?? "Errore avvio installazione MCP." }, { status: 500 })
  }
}
...
async function startDeviceFlow({
    ...
    return Response.json(
      {
        ...
        detail: "Aggiungi il provider a DEVICE_FLOW_DEFAULTS in app/api/mcp/connect/[connectorId]/route.ts",
      },
      { status: 501 },
    )
  }
  ...
  const scope = envValue(defaults.scopeEnv ?? "") || connector.graphUse?.join(" ") || ""
    ...
    return Response.json(
      {
        ...
        detail: "Il provider non ha restituito device_code e user_code. Riprova o controlla lo scope.",
        raw: deviceData,
      },
      { status: 502 },
    )
  ...
  const verificationUrl =
    deviceData.verification_url || deviceData.verification_uri || defaults.verificationUrl
  ...
  await upsertConnectorInstallation(auth.db, auth.principal, {
    ...
    config: {
      deviceFlow: {
        ...
        scope,
        interval,
        expiresAt: new Date(Date.now() + Number(deviceData.expires_in ?? 900) * 1000).toISOString(),
        startedAt: new Date().toISOString(),
